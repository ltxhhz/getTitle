...
var currentPage = 200001792, //开始的课程号
  maxPage = 300000000,//结束的课程号
  fault = [];
console.log('running');
...
function getData() {
  console.log('到', currentPage) /* 210897355 */
  getKnowledgeId(currentPage).then(async (res) => {
    let [a0, a1] = res, add;
    if (a1) {
      await addCourseToDB(currentPage, a1);
    }
    if (a0.length > 0) {
      let dataNow = a0[0],
        i = 1,
        page;
      add = async () => {
        getTitle(currentPage, dataNow).then(async (reBack) => {
          let [a, b, c] = reBack;
          ...
          if (a.length > 0) { //保存题目
            for (let j = 0; j < a.length; j++) {
              a[j] = a[j].trim();
              if (a[j].length > 2000) {
                await addTopicToDB(currentPage, a1, a[j].substring(0, 2000))
              } else {
                await addTopicToDB(currentPage, a1, a[j])
              }
            }
            if (dataNow != null) {
              await add();
              ...
              currentPage++
              if (currentPage === maxPage) { //上限
                console.log('获取完成');
              } else {
                console.log('下一课', currentPage);
                getData();
              }
            }
          } else { //跳过 没有题目
            if (c) { //记录 跳过
              fault.push(currentPage)
              fault = unique(fault)
              console.log('记录', fault)
            }
            if (dataNow) { //下一页
              await add()
            } else { //下一课
              console.log('跳过', currentPage)
              currentPage++
...
function addCourseToDB(courseId, courseName) {
  ...
  return new Promise((result, rej) => {
    conn.query('insert into courselist1 set ?', post, (err, res, fie) => {
      if (err) throw err;
      result(err)
    })
  })
...
async function addTopicToDB(courseId, courseName, topic) {
  ...
  new Promise((result, rej) => {
    conn.query('insert into topic1 set ?', post, (err, res, fie) => {
      if (err) throw err;
      result(err)
    })
  })
